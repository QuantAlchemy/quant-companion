...
function InvalidationResultCard({
  result,
  metricColumns,
}: {
  result: InvalidationResult
  metricColumns: 'sm:grid-cols-2' | 'sm:grid-cols-3'
}) {
  return (
    <article className="rounded-xl border border-border/80 bg-background/40 p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h3 className="text-lg font-semibold leading-tight">{result.name}</h3>
      ...
      <div className={cn('mt-4 grid gap-2', metricColumns)}>
...
export function StrategyInvalidationLab() {
  ...
  const runBenchmarkTests = async () => {
    ...
    try {
      const start = dateKey(tradeMetrics.dates[0])
      const end = dateKey(tradeMetrics.dates[tradeMetrics.dates.length - 1])
      ...
      <Collapsible open={isExpanded} onOpenChange={setLabOpen}>
        <CardHeader className="space-y-4">
          <div className="max-w-3xl space-y-2">
            ...
            <CardDescription>
              TradingView-export-first tests that attack the trade list you
              already uploaded. These do not prove a strategy is good; they try
              to prove it is not obviously fake, fragile, overfit, or
              outlier-driven.
            </CardDescription>
            ...
            <CardContent className="space-y-4 p-0">
              <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
                {report.results.map((result) => (
                  <InvalidationResultCard
                    key={result.id}
                    result={result}
                    metricColumns="sm:grid-cols-3"
                  />
                ))}
              </div>
              ...
              <div className="rounded-xl border border-border/80 bg-background/35 p-4">
                <h3 className="text-lg font-semibold">
                  Benchmark tests (live market data)
                </h3>
                <p className="mt-1 text-sm text-muted-foreground">
                  Buy-and-hold comparison and alpha/beta regression against a
                  benchmark, using Alpaca daily bars over your trade date range.
                  Use a stock/ETF symbol like SPY or QQQ, or a crypto pair like
                  BTC/USD.
                </p>
                <div className="mt-3 flex flex-wrap items-end gap-3">
                  ...
                  {tradeMetrics && tradeMetrics.dates.length < 2 && (
                    <span className="text-sm text-muted-foreground">
                      At least 2 trade dates are required.
                    </span>
                  )}
                </div>
                {benchmarkError && (
                  <p className="mt-3 rounded-lg border border-loss/40 bg-loss/10 p-3 text-sm text-loss">
                    {benchmarkError}
                  </p>
                )}
              ...
              <div className="rounded-xl border border-border/80 bg-background/35 p-4">
                <h3 className="text-lg font-semibold">
                  Roadmap for deeper invalidations
                </h3>
                <p className="mt-1 text-sm text-muted-foreground">
                  v1 stays intentionally trade-export-first. These are the next
                  data unlocks without pretending a TradingView trade list
                  contains signals it does not contain.
                </p>
                <Table className="mt-4">
                  ...
                  <TableBody>
                    <TableRow>
                      ...
                      <TableCell>
                        Enables random entry timing, random price paths,
                        benchmark overlays, and true path-dependent slippage
                        tests.
                      </TableCell>
                    </TableRow>
                    <TableRow>
                      ...
                      <TableCell>
                        Already supported by upload. The lab now summarizes
                        whether multiple uploaded files agree or expose
                        cherry-picking.
                      </TableCell>
                    </TableRow>
                    <TableRow>
                      ...
                      <TableCell>
                        TradingView Optimizer / browser automation
                      </TableCell>
                      <TableCell>
                        Could drive parameter reruns and randomized settings
                        later, but it is heavier and should stay out of v1.
                      </TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell className="font-medium">v3 option</TableCell>
                      <TableCell>
                        Strategy signal export / Pine randomization
                      </TableCell>
                      <TableCell>
                        Useful for true signal-shift tests, but unrealistic as a
                        first dependency for broad users.
                      </TableCell>
                    </TableRow>
                  </TableBody>
                </Table>
